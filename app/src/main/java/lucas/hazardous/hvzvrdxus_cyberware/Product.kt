    ...
    ListElementSurface {
        Column(modifier = Modifier.padding(8.dp)) {
            ...
            if (!editing) {
                ...
                TextField(value = price.toString(), onValueChange = { v ->
                    price = try {
                        v.toFloat().roundTwo()
                    } catch (nfe: NumberFormatException) {
                        1f
                    }
                }, label = { Text("Price") }, modifier = maxTextLengthModifier)
                TextField(
                    value = description,
                    onValueChange = { v -> description = v },
                    label = { Text("Description") },
                ...
                TextField(value = category.toString(), onValueChange = { v ->
                    category = try {
                        v.toInt()
                    } catch (nfe: NumberFormatException) {
                        0
                    }
                }, label = { Text("Category") }, modifier = maxTextLengthModifier)
            }
        }
        OutlinedButton(
            onClick = {
                if (editing && name.trim() != "" && price > 0f && description.trim() != "") {
                    val newProduct =
                        Product(product.id, name, price, description, category, product.image)
